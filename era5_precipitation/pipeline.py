...
@parameter(
    "start_date",
    name="Start date",
    help="Choose beginning of processing period",
    type=str,
    required=True,
)
@parameter(
    "end_date",
    name="End date",
    help="Choose end of processing period (last month by default)",
    type=str,
    required=False,
)
@parameter(
    "spatial_aggregation_level",
    name="Spatial aggregation level",
    help="Choose spatial aggregation level",
    choices=["Région", "District"],
    type=str,
    required=True,
    default="Région",
)
...
@parameter(
    "apply_weekly",
    name="Weekly",
    help="Apply weekly temporal aggregation",
    type=bool,
    required=False,
    default=True,
)
@parameter(
    "apply_monthly",
    name="Monthly",
    help="Apply monthly temporal aggregation",
    type=bool,
    required=False,
    default=True,
)
def era5_precipitation(
    ...
    # last day of previous month if no end date is set
    if end_date:
        ...
        end_date = datetime.datetime(
            dt.year, dt.month, monthrange(dt.year, dt.month)[1]
        )

    if spatial_aggregation_level == "District":
        boundaries = gpd.read_file(f"{workspace.files_path}/{config['districts']}")
    elif spatial_aggregation_level == "Région":
        boundaries = gpd.read_file(f"{workspace.files_path}/{config['regions']}")
    else:
        raise ValueError("Invalid spatial aggregation level")
    ...
    ds = merge(
        src_files=datafiles,
        dst_file=os.path.join(output_dir, f"{config['cds_variable']}.nc"),
    )
    ...
    df_daily = spatial_aggregation(
        ...
        dst_file=os.path.join(
            output_dir,
            f"{config['cds_variable']}_daily.csv",
        ),
    ...
    if apply_weekly:
        weekly(
            ...
            dst_file=os.path.join(
                output_dir,
                f"{config['cds_variable']}_weekly.csv",
            ),
        )
    ...
    if apply_monthly:
        monthly(
            ...
            dst_file=os.path.join(
                output_dir,
                f"{config['cds_variable']}_monthly.csv",
            ),
        )
